<div x-data="{
    ...
    get strengthColor() {
        ...
        return colors[this.strength] || 'bg-slate-200';
    },
    get strengthTextColor() {
        const colors = ['', 'text-rose-600', 'text-amber-600', 'text-indigo-600', 'text-emerald-600'];
        return colors[this.strength] || 'text-slate-400';
    }
}">
    <!-- Header -->
    <div class="mb-8 space-y-1.5">
        <h1 class="text-2xl font-black tracking-tight text-slate-900">Buat Akun Kosan</h1>
        <p class="text-sm text-slate-500 font-medium">Mulai kelola kos secara digital dalam 2 menit.</p>
    </div>
    ...
    <form wire:submit="register" class="space-y-4">
        ...
        <!-- Workspace/Boarding House Name -->
        <div class="space-y-1.5">
            <label for="workspace_name" class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider">Nama Kos / Properti</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
                    <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                </div>
                <input
                    ...
                    required
                    class="auth-input w-full pl-10 pr-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 text-sm font-medium"
                    placeholder="cth. Kosan Premium Cihampelas"
                >
            </div>
        ...
        <div class="space-y-1.5">
            <label for="password" class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider">Kata Sandi</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
            ...
            <!-- Strength bar -->
            <div x-show="password.length > 0" x-transition class="space-y-1.5">
                <div class="flex gap-1.5">
                    <template x-for="i in 4" :key="i">
                        <div class="flex-1 h-1.5 rounded-full bg-slate-200 overflow-hidden">
                            <div class="strength-bar h-full rounded-full" :class="i <= strength ? strengthColor : 'bg-transparent'" :style="'width:' + (i <= strength ? '100%' : '0%')"></div>
                        </div>
                    </template>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-[11px] font-semibold" :class="strengthTextColor" x-text="strengthLabel"></span>
                    <span class="text-[10px] text-slate-400 font-medium">8+ karakter, huruf besar, angka &amp; simbol</span>
                </div>
            </div>
        ...
        <!-- Confirm Password -->
        <div class="space-y-1.5">
            <label for="password_confirmation" class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider">Konfirmasi Kata Sandi</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
        ...
        <p class="text-[11px] text-slate-400 font-medium leading-relaxed">
            ...
            <a href="{{ route('terms') }}" class="text-indigo-600 font-semibold hover:underline">Ketentuan Layanan</a>
            dan
            <a href="{{ route('privacy') }}" class="text-indigo-600 font-semibold hover:underline">Kebijakan Privasi</a>
            kami.
        </p>
