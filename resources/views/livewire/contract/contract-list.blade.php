            ...
            @forelse($contracts as $ctr)
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center gap-2">
                            <x-button variant="outline" size="sm" class="px-2.5! py-1! text-xs font-semibold" onclick="window.location.href='{{ route('contracts.show', $ctr->id) }}'">
                                Manage Agreement
                            </x-button>
                            @if($ctr->status->value === 'draft')
                                <x-button variant="outline" size="sm" class="px-2.5! py-1! text-xs" onclick="window.location.href='{{ route('contracts.edit', $ctr->id) }}'">
                                    Edit
                                </x-button>
                            @endif
                            @can('delete', $ctr)
                                <x-button variant="outline" size="sm" class="px-2.5! py-1! text-xs text-rose-600 border-slate-200 hover:border-rose-100 hover:bg-rose-50 cursor-pointer" wire:click="confirmDelete('{{ $ctr->id }}')">
                                    Delete
                                </x-button>
                            @endcan
                        </div>
