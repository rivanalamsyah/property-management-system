<div class="space-y-6">
    ...
    <div>
            ...
            <div class="space-y-6">
                ...
                @if($tenant->subscription_status === \App\Enums\SubscriptionStatus::TRIAL)
                    <div class="p-6 bg-gradient-to-r from-indigo-50 to-violet-50 rounded-3xl border border-indigo-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
                        <div>
                            <h3 class="text-base font-bold text-indigo-900">You are currently on the Free Trial</h3>
                            <p class="text-sm text-indigo-750 mt-1 leading-normal">
                                Your professional tier trial expires in **{{ $trialRemainingDays }} days** ({{ $tenant->trial_ends_at?->format('d M Y') }}).
                            </p>
                        </div>
                ...
                @elseif($tenant->subscription_status === \App\Enums\SubscriptionStatus::CANCELLED)
                    <div class="p-6 bg-rose-50 rounded-3xl border border-rose-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h3 class="text-base font-bold text-rose-900">Subscription Cancelled</h3>
                            <p class="text-sm text-rose-750 mt-1 leading-normal">
                                Your account is scheduled to cancel. It remains active until **{{ $tenant->subscription_ends_at?->format('d M Y') }}**.
                            </p>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block">Current Plan</span>
                        <span class="text-2xl font-black text-slate-900 block mt-2">
                            {{ $tenant->subscriptionPlan ? $tenant->subscriptionPlan->name : 'No Plan selected' }}
                        </span>
                    ...
                    <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block">Renewal Date</span>
                        @if($tenant->next_billing_at)
                            <span class="text-2xl font-black text-slate-900 block mt-2">
                                {{ $tenant->next_billing_at->format('d M Y') }}
                            </span>
                            <span class="text-xs text-slate-400 block mt-3 leading-normal">
                                Auto-renews monthly using preferred method.
                            </span>
                        @else
                            <span class="text-2xl font-black text-slate-900 block mt-2">N/A</span>
                            <span class="text-xs text-slate-400 block mt-3 leading-normal">No upcoming renewal scheduled.</span>
                        @endif
                    </div>

                    <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block">Billing Method</span>
                        <span class="text-2xl font-black text-slate-900 block mt-2">Manual / Bank</span>
                        <span class="text-xs text-slate-400 block mt-3 leading-normal">Midtrans/Stripe integration prepared.</span>
                    </div>
                </div>

                <!-- Resource Usage Widget -->
                <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                    <h3 class="text-lg font-bold text-slate-900 tracking-tight">Workspace Resource Usage</h3>
                    <p class="text-xs text-slate-500 leading-normal mt-1">Resource allocation counts against your plan limits.</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                        ...
                        <div>
                            <div class="flex justify-between items-center text-sm font-semibold mb-2">
                                ...
                                <span class="text-slate-900">
                                    {{ $usage['rooms'] }} / {{ $tenant->getLimit('rooms') === -1 ? 'Unlimited' : $tenant->getLimit('rooms') }}
                                </span>
                            </div>
                        ...
                        <div>
                            <div class="flex justify-between items-center text-sm font-semibold mb-2">
                                ...
                                <span class="text-slate-900">
                                    {{ $usage['residents'] }} / {{ $tenant->getLimit('tenants') === -1 ? 'Unlimited' : $tenant->getLimit('tenants') }}
                                </span>
                            </div>
                            <div class="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                        ...
                        <div>
                            <div class="flex justify-between items-center text-sm font-semibold mb-2">
                                <span class="text-slate-700">Operational Staff</span>
                                <span class="text-slate-900">
                                    {{ $usage['staff'] }} / {{ $tenant->getLimit('staff') === -1 ? 'Unlimited' : $tenant->getLimit('staff') }}
                                </span>
                            </div>
                            <div class="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                        ...
                        <div>
                            <div class="flex justify-between items-center text-sm font-semibold mb-2">
                                <span class="text-slate-700">Storage Used</span>
                                <span class="text-slate-900">
        ...
        @elseif($activeTab === 'plan')
            <div>
                <h3 class="text-xl font-bold text-slate-900 tracking-tight">Select your Workspace tier</h3>
                <p class="text-sm text-slate-500 mt-1 leading-normal">Upgrade or switch plans dynamically. Upgrades take effect immediately.</p>
                    ...
                    @foreach($plans as $p)
                        <div class="bg-white border {{ $tenant->subscription_plan_id === $p->id ? 'border-indigo-600 ring-2 ring-indigo-500/20 shadow-md' : 'border-slate-100 shadow-sm' }} rounded-3xl p-6 flex flex-col justify-between hover:scale-[1.01] transition-all relative overflow-hidden">
                            @if($tenant->subscription_plan_id === $p->id)
                                <div class="absolute top-0 right-0 bg-indigo-600 text-white text-[9px] font-black tracking-widest px-3 py-1 uppercase rounded-bl-xl">Current Plan</div>
                            @endif
                            ...
                            <div>
                                ...
                                <ul class="space-y-2.5 text-xs text-slate-600 font-medium">
                                    <li class="flex items-center gap-2">
                                        ...
                                        <span>Max Rooms: {{ $p->max_rooms === -1 ? 'Unlimited' : $p->max_rooms }}</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                        <span>Max Residents: {{ $p->max_tenants === -1 ? 'Unlimited' : $p->max_tenants }}</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                        <span>Storage limit: {{ $p->storage_limit_mb }}MB</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                        <span>Portal: {{ $p->has_resident_portal ? 'Included' : 'No' }}</span>
                                    </li>
                                </ul>
                            </div>
        ...
        @elseif($activeTab === 'settings')
            <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                <h3 class="text-lg font-bold text-slate-900 tracking-tight">Workspace configuration</h3>
                <p class="text-xs text-slate-500 leading-normal mt-1">Configure legal entities, localized formats, and branding slugs.</p>

                <div class="mt-8 space-y-6 max-w-xl">
                    <div>
                        <x-label for="name">Workspace Name</x-label>
        ...
        @elseif($activeTab === 'history')
            <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                <h3 class="text-lg font-bold text-slate-900 tracking-tight">Workspace Audit Log</h3>
                <p class="text-xs text-slate-500 leading-normal mt-1">Audit log records of every SaaS operation triggered inside this workspace.</p>
                ...
                <div class="mt-6 border border-slate-50 rounded-2xl overflow-hidden overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100 text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-8 text-center text-slate-400 italic">No operations recorded yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
